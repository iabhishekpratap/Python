...
'''count =0
fin=open("f:\\file.txt","r")   # write the location of file
fout=open("f:\\new.txt","w") # for creating new file
data=fin.readlines()
for line in data:
       if 'a'  not in line:
               print(line,end="")
               fout.write(line)   # contents to be written in new file 
               count=count+1
...
#print(size)
'''
#Reading specific no. of bytes
'''
f=open('c:\poem\dragon.txt')
print("before reading data ", f.tell())  # tell() tells the position of file pointer
data=f.read()
print("after reading complete data ", f.tell())
f.seek(0)     # seek() moves the file pointer to specific no. of bytes
print("File Pointer is at  ->  ", f.tell())
data=f.read(6)
print("6 bytes File Pointer is at  ->  ", f.tell())
data=f.read(20)
print("next 10 bytes File Pointer is at  ->  ", f.tell())
f.seek(-10,2)
print("File Pointer is at  ->  ", f.tell())
f.close()
'''
(35.)#To get rnos, names and marks of the students and store these details in a file called "Marks.dat"
'''
...
f=open("nfile.txt","w")
num = int(input ("\nEnter the a Number "))
f.write(str(num))                       # CONVERT NUMERIC DATA INTO STRING
f.close()
...
f.close()
'''
#(29.) A PYTHON PROGRAM TO COUNT ALL THE LINE HAVING 'a' AS LAST CHARACTER
'''
count =0
f=open("c:\poem\dragon.txt","r")
data=f.readlines()
...
for line in data:
    print(line)
    if line[-1] == '': #In Binary File last character of the line is at -2 position count=count+1
        count+=1
print("Number of lines having 'a' as last character is/are : " ,count)
f.close()
'''
#(28.) A PYTHON PROGRAM TO COUNT ALL THE LINE HAVING 'A' AS FIRST CHARACTER
'''
count =0
f=open("c:\poem\dragon.txt","r")
data=f.readlines()
...
for line in data:
        if line[0] == 'A':
                count=count+1
print("Number of lines having 'A' as first character is/are : " ,count)
f.close()
'''
...
data=f.read()
for c in data:
        if c== 'a' or c=='e' or c=='i' or c=='o'or c=='u':      # Check each character for a vowel
                count=count +1
print("The number of Vowels in this file are : ",count)
f.close()
'''
#(24.) A python program to find the number of characters in a text file
'''f=open('c:\poem\dragon.txt',"r")
...
for line in linesList:                      # Read each line
        wordsList=line.split()          # THIS FUNCTION IS USED TO BREAK THE LINE INTO LIST OF WORDS
...
fin.close()
'''
#(13.)Reading n bytes and then reading some more bytes from the last position
'''fin=open('c:\poem\dragon.txt',"r")
str=fin.read(10)
print(str)
str=fin.read(5)
...
f.close()
'''
#(7.)Program to write  data to a file using writelines method() and "with" command
'''
with open("f://student.txt","w") as f :
    lst=["vivek\n","christafar\n","ansh","saanidhya","manu\n","devansh\n","shivangi","Himanshi","abhishek"]
    f.writelines(lst)
